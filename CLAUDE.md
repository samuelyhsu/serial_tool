...
- **状态栏是界面上唯一的秒级时钟**（`ui/StatusBar`）。运行时长、收发速率、写队列积压
  都搭这一趟 `setInterval`，而且**端口关闭时它根本不启动**（缺陷 D8：空闲时整个界面
  必须是静止的）。往状态栏加读数前先分清它属于哪一类：端口关着时也得是对的那些
  （过滤命中）必须走 store 订阅，挂到这趟时钟上会停在最后一个值。
  **过滤命中数是接收区回推的**（`logStore.setFilterMatches`）—— `selectRows` 是单槽
  记忆化的，状态栏自己再算一遍等于两处查询条件互相顶掉对方的缓存，每帧重扫两趟缓冲。
  状态栏还收着几个本属别处的控件：时间列、分帧（下拉框 + 空闲时长）、日志容量、
  语言与主题。判断一个控件该不该放这儿，看它是不是「连接后不再可改的事实」或
  「装一次就不再碰的设置」；高频动作（暂停、清空、过滤、视图开关）留在就近的工具栏上。
  **这一栏的稀缺资源是横向空间**：一折行就吃掉接收区的高度。所以只表达一个开关状态的
  控件（自动重连 `⟳`、仅匹配 `∇`、正则 `.*`）一律只画图标，把说明挂在 title 上 ——
  图标省的是宽度，不是说明，`aria-label` 与 `title` 两者都不能省。
