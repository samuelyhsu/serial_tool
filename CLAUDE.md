...
- **状态栏是界面上唯一的秒级时钟**（`ui/StatusBar`）。运行时长、收发速率、静默时长、
  写队列积压都搭这一趟 `setInterval`，而且**端口关闭时它根本不启动**（缺陷 D8：空闲时
  整个界面必须是静止的）。往状态栏加读数前先分清它属于哪一类：端口关着时也得是对的
  那些（缓冲占用、过滤命中）必须走 store 订阅，挂到这趟时钟上会停在最后一个值。
  **过滤命中数是接收区回推的**（`logStore.setFilterMatches`）—— `selectRows` 是单槽
  记忆化的，状态栏自己再算一遍等于两处查询条件互相顶掉对方的缓存，每帧重扫两趟缓冲。
  状态栏还收着两个本属别处的控件：分帧（下拉框 + 空闲时长）与日志容量。判断一个控件
  该不该放这儿，看它是不是「连接后不再可改的事实」或「装一次就不再碰的设置」；
  高频动作（暂停、清空、过滤、视图开关）留在就近的工具栏上。
