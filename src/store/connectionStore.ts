...
/**
 * 波特率候选值：以下几份档位表的并集，再加上几个协议 / 设备特定值。
 *  - Windows `DCB` 的 `CBR_*` 常量：110 ~ 256000
 *  - Linux termios 的 `B*` 常量：50 ~ 4000000（含 SPARC 专有的 76800 / 153600 / 307200 / 614400）
 *  - Silicon Labs CP210x AN205 Table 1（Linux `cp210x.c` 的 `cp210x_an205_table1`）：
 *    4000 / 16000 / 51200 / 64000 等只有它才列的档
 *  - WCH CH340 手册：100 / 900 / 3600 / 33600 等（134.5 不是整数，由 134 代表）
 *  - 协议 / 设备：31250 MIDI；74880 ESP8266 上电日志；250000 / 500000 / 1000000 DMX512、
 *    3D 打印机（Marlin）、Dynamixel 舵机
 *
 * 这只是下拉建议，不是限制 —— 输入框接受任意值，具体能不能开由驱动决定。
 */
export const BAUD_RATES = [
  50, 75, 100, 110, 134, 150, 200, 300, 600, 900, 1200, 1800, 2400, 3600, 4000, 4800, 7200, 9600,
  14400, 16000, 19200, 28800, 31250, 33600, 38400, 51200, 56000, 57600, 64000, 74880, 76800, 115200,
  128000, 153600, 230400, 250000, 256000, 307200, 460800, 500000, 576000, 614400, 921600, 1000000,
