...
- **串口参数**：波特率 51 档常用值（50 ~ 4000000，汇总 Windows / Linux 标准档与 CP210x、CH340 芯片档位表，含 MIDI 31250、ESP8266 74880、DMX/3D 打印 250000 等），点开下拉列出全部档位、边打字边按前缀筛选，也支持直接输入任意自定义值
...
- **日志缓冲**：环形缓冲，容量默认 10000 条、可在接收区工具栏改（不少于 1000，上不封顶），
  超出后自动丢弃最旧的记录；界面渲染最新 1000 行，更早的那些在列表顶部标明条数，
  可用「保存」导出或用过滤查看。**容量越大越吃内存**——单帧上限 8 KB，最坏占用就是
  容量 × 8 KB，调到几十万条前先掂量一下
...
| `logCapacity`        | 全局   | 日志缓冲容量（≥ 1000，默认 10000，无上限）                           |
...
- 日志虚拟滚动 —— 缓冲里默认存着 10000 条（可调更大），但界面只渲染最新 1000 行，
  更早的目前只能导出或过滤查看。上了虚拟滚动才能整个滚起来，代价是要重写
  「贴底判定 + 自动滚屏」那套直接读 scrollHeight 的逻辑
- DTR / RTS 信号控制与 Break
- 日志多格式导出（CSV、二进制）
- 数据波形绘图
