...
const REQUESTS: Record<RequestMethod, RequestBody> = {
  ...
  'session.setSignals': { method: 'session.setSignals', signals: { dataTerminalReady: false } },
  'session.sendBreak': { method: 'session.sendBreak', durationMs: 250 },
  'session.getSignals': { method: 'session.getSignals' },
